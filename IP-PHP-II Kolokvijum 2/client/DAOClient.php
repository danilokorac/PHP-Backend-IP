...
class DAOClient {
    ...
    private $CLIENTEXIST = "SELECT * FROM klijent WHERE user = ? AND pass = ?";
    private $UPDATECLIENT = "UPDATE klijent SET logInTime = CURRENT_TIMESTAMP WHERE user = ? AND pass = ?";
    ...
    public function  clientExist($user, $pass) {

        $statement = $this->db->prepare($this->CLIENTEXIST);
    ...
    public function  updateLogInTime($user, $pass) {
        ...
        $statement = $this->db->prepare($this->UPDATECLIENT);
        $statement->bindValue(1, $user);
		$statement->bindValue(2, $pass);
